...
app.post("/session/screenshot", async (req, resp) => {
    ...
    Logger.info(`Try to get Session Result,  result is ${respBody.type}${respBody.type === "Failed" ? `(${respBody.state})` : ""}. Session Id: ${sessionId}.`);
    resp.json(respBody);
    return;
});
...
app.post("/session/action", async (req, resp) => {
    ...
    Logger.info(`Given A instruction to Session, result is ${respBody.type}${respBody.type === "Failed" ? `(${respBody.state})` : ""}. Session Id: ${sessionId}.`);
    resp.json(respBody);
    return;
});
