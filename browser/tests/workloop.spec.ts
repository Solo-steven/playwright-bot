...
import { ActionInstruction, ActionResult, ActionInstructionType, BrowserSessionState } from "../src/type";
...
async function setActionResult(sessionId: string, base64Image: string, errorMsg: string) {
  if(errorMsg) {
    const errorResp: ActionResult = { type: "Error", content: errorMsg, base64EncodeString: base64Image };
    ...
    const actionResp: ActionResult = { type: "Success", base64EncodeString: base64Image };
    await redisClient.multi()
      .set(`${sessionId}_state`, BrowserSessionState.Result)
