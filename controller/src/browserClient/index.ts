import { ActionInstruction, ActionInstructionAPIResponseBody, ActionResult, ActionResultAPIResponseBody, BrowserSessionState } from "./type";
...
export async function createBrowserClient(url: string): Promise<BrowserClient> {
    ...
    async function getResult() {
        while(1) {
            const resp: ActionResultAPIResponseBody = await fetch(`${browserServerURL}/session/screenshot`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json"
                },
    ...
    async function sendInstruction(actionInstruction: ActionInstruction) {
        while(1) {
            const resp: ActionInstructionAPIResponseBody = await fetch(`${browserServerURL}/session/action`, {
                method: "POST",
                headers: {
                    "Content-Type": "application/json"
                },
