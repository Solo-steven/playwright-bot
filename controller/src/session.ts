...
export async function createSession(config: SessionConfig): Promise<Session> {
    ...
    return {
      ...
      bucketClient: createBucketClient({ bucketName: config.bucketName, credentials: config.credentials }),
      config,
...
export async function createSessionWorker(session: Session) {
    ...
    try {
        while(1) {
            if(iterTime > session.config.maxIter) {
                return;
            }
            const result = await session.browserClient.getResult();
            const url = await saveBased64ImageIntoBucket(session, result, iterTime);
            const browserMsg = await getLLMMessageFromActionResult(result, url, iterTime, session.config.taskDescription);
